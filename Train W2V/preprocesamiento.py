...
class TextProcessor:
    ...
    def __init__(self):
        ...
        additionalStopwordsSpanish = {'u','y/o','año','años','alto','ser','etc','respecto','hacer','tal','dentro','mes','meses','tener','experiencia','conocimiento','trabajo','bueno','afín','afine',
    ...
    def lematizeText(self,text):#Para analizar frases y mejorar el desempeno del lemmatizador
    	newText = ""
    	lemmaElement = 2 
    	...
    	for sentence in sent_tokenize(text,language='spanish'): 
    		...
    		#text=self._cleanhtml(textWithSeparatePunctuation)
    		...
    		for wordLemmaResult in lemmaResultText:
    			#print(wordLemmaResult)
    			wordElements = wordLemmaResult.split('\t')
    			if len(wordElements)>1:
    				word = wordElements[lemmaElement].lower().strip()    			
...
class Input_Output:
    ...
    def limpiarDataset_Stemming_Bigram(self, dataset,thresholds,years,months):
    	bigrams = []
    	for threshold in thresholds:
    		bigrams.append(Phrases(threshold=threshold))
    	newDataset = []
    	dates = {}
    	for iOferta in range(len(dataset)):
    		ofertaText = self.procesadorTextos.preprocessText(dataset[iOferta])
    		#ofertaText = self.procesadorTextos.stemText(ofertaText)
    		...
    		if len(listaPalabras)>4:
    			...
    			print('Stem: No se tomo en cuenta la oferta '+ str(iOferta) +' '+ str(listaPalabras))
    	...
    	for year in sorted(dates.keys()):
    		for month in sorted(dates[year].keys()):
    			print(str(year)+'/'+str(month)+': '+str(dates[year][month]))
    	return newDataset,bigrams,dates

    def limpiarDataset_Lemmatizacion_Bigram(self, dataset,thresholds,years,months):
    	bigrams = []
    	for threshold in thresholds:
    		bigrams.append(Phrases(threshold=threshold))
    	dates = {}
    	...
    	numOferta = 1
    	for iOferta in range(len(dataset)):  		
    		cadenaLemmatizada = self.procesadorTextos.lematizeText_Filter_TypeWord(dataset[iOferta].strip())
    		#cadenaLemmatizada = self.procesadorTextos.lematizeText(oferta.strip())
    		cadenaPreProcesadaLemmatizada = self.procesadorTextos.preprocessText(cadenaLemmatizada)
    		...
    		if len(listaPalabras)>4:
    			...
    			print("Lem: No se tomo en cuenta la oferta "+ str(iOferta) +' '+ str(listaPalabras))
    ...
    def obtenerDatasetAClasificar_Completo_Bigram(self, filename,thresholds_Lemma,thresholds_Stem):
        ...
        for num_oferta in range(1, maxFilas):
            ...
            dataset.append(text)
            year = int(sheetAviso.cell(row=num_oferta, column=yearColumn).value)
            month = int(sheetAviso.cell(row=num_oferta, column=monthColumn).value)
            years.append(year)
            months.append(month)

        datasetLemmatizacion,bigramsLemmatizacion,datesLemmatizacion = self.limpiarDataset_Lemmatizacion_Bigram(dataset,thresholds_Lemma,years,months)
        datasetSteeming,bigramsSteeming,datesStemming = self.limpiarDataset_Stemming_Bigram(dataset,thresholds_Stem,years,months)

        pickle.dump(datesLemmatizacion,open("datesLemmatizacion_" + 'Informatica' + ".p","wb"))
        pickle.dump(datesStemming,open("datesStemming_" + 'Informatica' + ".p","wb"))
